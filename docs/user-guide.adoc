...
Router allows to integrate browser navigation into you app. In the router you define bidirectional mapping between the state and the Uri (about migration from Path routing read below).
...
router = Router(
    ...
    toState = {
// match / without any parameters
      case Root :? _ =>
        initialState =>
          Future.successful(initialState)
// match /search with two optional parameter `begin` and `end`
...
https://github.com/fomkin/korolev/blob/v0.17.0/examples/routing/src/main/scala/UriRoutingExample[See full example]

For easy parameter matching Korolev provide simple DSL functions and parameter extractor:

* `:?` - begin parameters matching block
* `+&` - add one more parameter to match
* `QueryParam` - abstract class for matching required parameter
* `OptionQueryParam` - abstract class for matching optional parameter

Also you can simple do `case Root / .. / .. :? params =>` code `:? params` return all parameters as `Map[String, String`
...
==== Migration existing Router to Uri routing

For migration your routing's from previous version of Korolev you need to do two things:

* Add import of new class Uri with implicit conversion from Path to Uri, for correct work of `fromState`.
[source,scala]
...
* Cover existing `toState` code with `Uri {}` for correct convert `PartialFunction[Path, S => F[S]]` to `PartialFunction[Uri, S => F[S]]`.
[source,scala]
...
https://github.com/fomkin/korolev/blob/v0.16.0/examples/evalJs/src/main/scala/EvalJs.scala[See full example]
