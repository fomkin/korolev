...
class PathAndQuerySpec extends FlatSpec with Matchers {
  ...
  ".fromString" should "parse path with RelativeRoot" in {
    val path = PathAndQuery.fromString("page/1")
    path shouldBe RelativeRoot / "page" / "1"
  }
  ...
  ".mkString" should "make from path with RelativeRoot" in {
    val path = RelativeRoot / "page" / "1"
    path.mkString shouldBe "page/1"
  }
  ...
  ".mkString" should "make from path with many parameters" in {
    val path = Root / "page" / "1" :? =?=("k1", "v1") :& =?=("k2", "v2") :& =?=("k3", "v3") :& =?=("k4", "v4") :& =?=(
      "k5",
      "v5")
  ...
  ".++" should "correct concatenate simple path" in {
    val head = Root / "page"
    val tail = RelativeRoot / "1"

    head ++ tail shouldBe Root / "page" / "1"
  }

  ".++" should "correct concatenate RelativeRoot path" in {
    val head = RelativeRoot / "page"
    val tail = RelativeRoot / "1"

    head ++ tail shouldBe RelativeRoot / "page" / "1"
  }

  ".++" should "correct concatenate complex path" in {
    val head = Root / "api" / "v1" / "system"
    val tail = RelativeRoot / "admin" / "parameters" / "edit"
  ...
  ".reverse" should "correct reverse RelativeRoot path" in {
    val path = RelativeRoot / "api" / "v1" / "system"

    path.reverse shouldBe RelativeRoot / "system" / "v1" / "api"
  }
  ...
  "path matching" should "correct exact match many parameter" in {
