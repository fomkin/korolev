...
private[korolev] final class SessionsService[F[_]: Effect, S: StateSerializer: StateDeserializer, M](
  ...
  def initAppState(qsid: Qsid, rh: RequestHeader): F[S] =
    for {
      ...
      _ <- stateStorage.create(qsid.deviceId, qsid.id, state)
    } yield state
  ...
  private def appsFactory(qsid: Qsid, rh: RequestHeader, incoming: Stream[F, String]) =
    for {
      exists <- stateStorage.exists(qsid.deviceId, qsid.id)
      stateManager <-
        if (exists) stateStorage.get(qsid.deviceId, qsid.id)
        else initAppState(qsid, rh).flatMap(stateStorage.create(qsid.deviceId, qsid.id, _))
      ...
      fromScratch = !exists
      frontend = new Frontend[F](config.reporter, incoming)
      ...
      incoming.consumed.run { _ =>
        stateStorage.remove(qsid.deviceId, qsid.id)
        apps.remove(qsid)
      } // TODO use .start
